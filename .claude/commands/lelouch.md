...
**ANTES de interpretar qualquer input, SEMPRE executar:**

```powershell
c:\git\pessoal\squad\scripts\validate-kanban.ps1 -ProjectPath (Get-Location)
```

- ✅ Se kanban existe e é válido → Continue normalmente
- ❌ Se kanban não existe → Ofereça bootstrap
- ⚠️ Se kanban inválido → Mostre quais campos estão quebrados

## PRÉ-AÇÃO OBRIGATÓRIA: Dry-run de roteamento

Após validar o kanban, SEMPRE execute o roteador para justificar convocação:
...
Regras:
- use o score para priorizar os especialistas da fase
- respeite os excluídos por `notFor`
- registre no plano os agentes recomendados e os agentes não convocados
- se o modo sugerido for `single-agent`, evite paralelismo desnecessário
- mantenha no log o bloco `DRY_RUN_SUMMARY_*` sem alterar formato
